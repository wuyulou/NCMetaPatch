...
| 发行日期 | 尽量统一为 `YYYY-MM-DD`，输入日期框后会触发一次回填 |
| 专辑类型、曲风 | 有可靠识别结果时尝试匹配，仍需人工确认 |
| 专辑介绍 | 使用草稿中的来源说明或补充备注文字，不写入曲目列表 |
| 独立“补充备注” | 永远不自动填写 |
...
NCMetaPatch is a Chrome/Edge MV3 extension for collecting album metadata from Discogs, Rate Your Music, and Bandcamp, then helping fill the NetEase Cloud Music album submission page.
